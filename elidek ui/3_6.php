...
<body>
   ...
   <h2>Young Researchers</h2>
   <?php
    echo "<br>";
    
    $query = "SELECT researcher.researcher_name, COUNT(researcher_in_project.project_id) AS number_of_participations, datediff(curdate(), researcher.birthdate) div 365 
    FROM researcher 
    ...
    if(mysqli_num_rows($result) == 0){
        echo '<h1 style="margin-top: 5rem;">No Young Researchers found!</h1>';
    }
    else{
        ...
        echo '<table class="table" style="border:1px solid black;margin-left:auto;margin-right:auto;">';
            echo '<thead>';
                echo '<tr>';
                    echo '<th style="width:40%">Full Name</th>';
                    echo '<th style="width:20%">Age</th>';
                    echo '<th style="width:20%">Number of Active Projects</th>';
                echo '</tr>';
            echo '</thead>';
            echo '<tbody>';
