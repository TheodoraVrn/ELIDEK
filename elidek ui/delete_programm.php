...
<?php
        $id = $_GET['id'];
                    $query = "SELECT * FROM programm WHERE programm_id = $id";
                    $res1 = mysqli_query($conn, $query);
                    $row = mysqli_fetch_row($res1);

                    echo '<div class="form-group col-sm-3 mb-3">';
                        echo '<label class = "form-label" style="width: 300px;">Are you sure you want to delete programm <br><b>' . $row[0] . ' ' . $row[1] . '?</b></label>';
                        echo '<label class = "form-label" style="width: 300px;"> <br><br> (Note: Please make sure you have updated/deleted the projects this programm funds, so that it does not fund any projects, or else a fatal error will happen)</label>';
                    echo '</div>';
?>  
                     <br>
